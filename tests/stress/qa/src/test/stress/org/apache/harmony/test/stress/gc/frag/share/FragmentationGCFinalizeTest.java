...
public class FragmentationGCFinalizeTest extends FragmentationAbstractTest {

    volatile int finCount = 0;
    ...
    public void perform() {
        ...
        if (finCount < ((int) (NUM / PERIOD))) {
            ReliabilityRunner.debug("Finalize function was invoked only "
                    + finCount + " time(s). Expected value "
                    + ((int) (NUM / PERIOD)) + ". Total size = " + totalSize
                    + " i = " + i);
    ...
    class FinalizedObject {
        FragmentationGCFinalizeTest test = null;

        byte[] bytes = null;

        FinalizedObject(FragmentationGCFinalizeTest test, int size) {
            this.test = test;
            bytes = new byte[size];
        }
