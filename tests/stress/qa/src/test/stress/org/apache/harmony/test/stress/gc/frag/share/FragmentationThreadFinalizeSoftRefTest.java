...
public class FragmentationThreadFinalizeSoftRefTest extends
    ...
    public static final int SOFTREF_COUNT = 1024;
                ...
                for (; i < NUM; ++i) {
                    ...
                    if (size >= MIN_BLOB_SIZE) {
                        ...
                        objects[i] = new FinalizedObject(
                                FragmentationThreadFinalizeSoftRefTest.this,
                                size);
                    }
                    count++;
                    ...
                    if (count > PERIOD) {
                        ...
                        for (int j = start; j < i; j += step) {
                            if (objects[j] != null) {
                                ...
                                if (((j + i) % SOFTREF_PORTION) == 0) {
                                    if (indexSR == sr.length) {
                                        indexSR = 0;
                                    }
                                    // System.out.println("j = " + j + ",
                                    // indexSR = " + indexSR);
                                    sr[indexSR++] = new SoftReference(
                                            objects[j]);
    ...
    class FinalizedObject {
        FragmentationThreadFinalizeSoftRefTest test = null;

        byte[] bytes = null;

        FinalizedObject(FragmentationThreadFinalizeSoftRefTest test, int size) {
            this.test = test;
            bytes = new byte[size];
        }
