...
public final class String implements Serializable, Comparable, CharSequence {
	...
	static class ConsolePrintStream extends java.io.PrintStream {
		 ...
		 */
		public void print(String str) {     //////wjw
			if (str == null)
				str = "\n";  ///wjw a temporary hack
            //Java_com_ibm_platform_OSFileSystem_writeImpl()
            //jobject obj1, jint theCharacterToBeWritten)
            //private final char[] value;
	        //private final int offset;
	        //private final int count;
            //public long write(long fileDescriptor, byte[] bytes, int offset, int length)
            if ( (str.count + str.offset) > str.value.length) {
            ...
            {
                ba[ii] = (byte)str.value[ii];
            }
            System.out.write(ba, str.offset, str.count);           
		}
	}
